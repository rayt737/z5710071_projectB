...
- Every report number must trace to one of the CSVs under `results/`
  ...
  `results/tables/vol_target_comparison.csv`). Never build a figure/table from
  a hand-computed number.
- fund ids are `{family}_{method}` and BOTH sides can contain underscores
  (`min_variance`, `max_sharpe`, `risk_parity`, `growth_sensitive`). Parse ids
  ONLY with `portfolios.parse_fund_id` (method inferred by known-suffix match,
  family-agnostic); never `split`/`rsplit` directly - see
  `ai/prompt_log.md` Entries 2 and 6.
